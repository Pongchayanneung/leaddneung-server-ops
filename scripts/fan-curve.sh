#!/bin/bash
# runs as root (systemd oneshot) — write directly to sysfs
# Resolve hwmon by name: the index (hwmonN) is NOT stable across boots.
set -euo pipefail
...
if [ -z "$H" ]; then
  echo "fan-curve: asus_custom_fan_curve hwmon not found" >&2
  exit 1
fi
